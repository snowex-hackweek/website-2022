...
if [[ $? -eq 1 ]]; then
  conda create -q -y -n ${LOCK_ENV} -c conda-forge conda-lock=1.0.5 mamba=0.23
fi
...
echo '    - uavsar_pytools === 0.6.1' >> conda-linux-64.lock.yml
echo '    - uavsar_pytools === 0.6.1' >> conda-osx-64.lock.yml
